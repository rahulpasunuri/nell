...
import static similarity.CharacterVectorUtils.*;
import static similarity.VectorMath.*;
...
public class CosineSimilarity implements ISimilarityCalculator {
    ...
    public double calculate(String stringOne, String stringTwo) {
        Collection<Character> unionOfStringsOneAndTwo = union(stringOne,
                stringTwo);
        Collection<Integer> stringOneOccurrenceVector = createFrequencyOfOccurrenceVector(
                stringOne, unionOfStringsOneAndTwo);
        Collection<Integer> stringTwoOccurrenceVector = createFrequencyOfOccurrenceVector(
                stringTwo, unionOfStringsOneAndTwo);

        int dotProduct = 0;
        // This should be an unnecessary exception since we're submitting the
        // union
        // of both strings
        try {
            dotProduct = dotp(stringOneOccurrenceVector,
                    stringTwoOccurrenceVector);
        } catch (similarity.VectorMathException e) {
            e.printStackTrace();
     ...
     *            Input String
     * @param unionSet
     *            Set of all Character-dimensions
     * @return Frequency of Occurrence Vector
     */
    private static Collection<Integer> createFrequencyOfOccurrenceVector(
        ...
        for (Character c : unionSet) {
            occurrenceVector.add(countCharacter(string, c));
        }
